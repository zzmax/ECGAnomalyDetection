                   ...
                   double usualStd, double anomalies[6])
{
    ...
    double s;
    emxArray_boolean_T *c_reconstructDelta;
    ...
    for (d = 0; d < n; d++) {
        b_reconstructDelta->data[d] = (reconstructDelta->data[d] > y);
    }

    emxInit_boolean_T(&c_reconstructDelta, 2);
    d = c_reconstructDelta->size[0] * c_reconstructDelta->size[1];
    c_reconstructDelta->size[0] = 1;
    c_reconstructDelta->size[1] = reconstructDelta->size[1];
    emxEnsureCapacity((emxArray__common *)c_reconstructDelta, d, (int)sizeof
            (boolean_T));
    y = usualMean - 4.0 * usualStd;
    n = reconstructDelta->size[0] * reconstructDelta->size[1];
    for (d = 0; d < n; d++) {
        c_reconstructDelta->data[d] = (reconstructDelta->data[d] < y);
    }

    emxInit_boolean_T(&b_m, 2);
    outliers = sum(b_reconstructDelta) + sum(c_reconstructDelta);
    ...
    y = m + s;
    n = reconstructDelta->size[0] * reconstructDelta->size[1];
    emxFree_boolean_T(&c_reconstructDelta);
    ...
    /*   */
    /*  window = buffer(reconstructDelta,1001,1000); */
    /*  window = window(:,1001:end); */
    mean(window, r1);
